...
#TABLES="warehouse district customer history item orders new_order stock order_line"
TABLES="orders new_orders  stock order_line"
#TABLES="item   stock order_line"
...
for TABLE in $TABLES ; do
  ...
  command_exec "$MYSQL $DB_NAME -e \"\$$TABLE ENGINE=$DB_ENGINE ROW_FORMAT=DYNAMIC \""
...
while getopts "d:h:f:s:c:e:m:u:p:o:vl" opt; do
        case $opt in
        c)
                MYSQL=$OPTARG
                ;;
...
#if [ "$DB_ENGINE" != "INNODB" -a "$DB_ENGINE" != "MYISAM" -a "$DB_ENGINE" != "FALCON" ]; then
#  usage "$DB_ENGINE. Please specifey correct storage engine [MYISAM|INNODB|FALCON]"
#  exit 1
#fi
...
if [ ! -f "$MYSQL" ]; then
  usage "MySQL client binary '$MYSQL' not exists.
       Please adjust PATH variable or specify correct one using -c #"
  exit 1
fi
...
if [ "$LOCAL" != "" ]; then
  MYSQL_ARGS="$MYSQL_ARGS --local-infile "
fi
