...
$medium_loop_count=$small_loop_count*10;         # 
...
if ($max_table_size >0)
{
   $bench1_recordlen = 42; # record length of bench1 table.
   $opt_loop_count = int (($max_table_size / $bench1_recordlen)/3) ;
   $many_keys_loop_count=$opt_loop_count;
   $opt_read_key_loop_count=$opt_loop_count;
}
...
if ($random_loop_count > $opt_loop_count) 
{
  $random_loop_count = $opt_loop_count;
}


$opt_loop_count = floor($opt_loop_count/$random_loop_count +0.5)*$random_loop_count;
$total_rows=$opt_loop_count*3;

print "Testing the speed of inserting data into 1 table and do some selects on it.\n";
print "The tests are done with a table that has $total_rows rows.\n\n";
...
$end_time=My::Timer::get_timer();
print "Time for insert (" . ($opt_loop_count) . "): " .
  My::Timer::timestr(timediff($end_time, $loop_time),"all") . "\n\n";
...
{ print "Time"; }
print " for order_by_big_key_prefix ($medium_loop_count:$rows): " .
  My::Timer::timestr(timediff($end_time, $loop_time),"all") . "\n";
...
$rows=0;
$count=0;
for ($i=1 ; $i <= $range_loop_count*10 ; $i++)
{
  my $rnd=$i;
  my $rnd2=$random[$i];
