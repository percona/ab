...
sub get_server
{
  my ($name,$host,$database,$odbc,$machine,$socket,$connect_options,$in_cmp)=@_;
  my ($server);

  #Due the fact that we call get_server from both test-* and crash-me
  #independenly, we put check below here
  $main::opt_odbc=$odbc=1 if (!$opt_odbc && $name =~ /_ODBC$/i);

  if ($name =~ /mysql|mysqlEmb/i)
  { $server=new db_MySQL($host, $database, $machine, $socket,$connect_options,$in_cmp, $name); }
  elsif ($name =~ /pg/i)
  ...
  elsif ($name =~ /sybase-asa/i)
  { $server= new db_sybase_asa($host,$database); }
  elsif ($name =~ /sybase-ase/i)
  { $server= new db_sybase_ase($host,$database); }
  ...
  elsif ($name =~ /Sapdb|maxdb/i)
  { $server= new db_sapdb($host, $database, $socket, $connect_options, $name); }
  elsif ($name =~ /sqlite/i)
  { $server= new db_sqlite($host,$database); }
  elsif ($name =~ /interBase/i)
  { $server= new db_interbase($host,$database); }
  elsif ($name =~ /firebird/i)
  { $server= new db_firebird($host,$database); }
  else
  {
      die "Unknown sql server name used: $name\nUse one of: Access, Adabas, AdabasD, Empress,".
        " FrontBase, Oracle, Informix, InterBase, DB2, mSQL, Mimer, MS-SQL, MySQL, Pg, Solid, ".
	"SQLite, SAPDB, MaxDB, MaxDBProxy, Sybase or firebird.\nIf the connection is done trough ODBC the name must end with _ODBC\n";
  }

  #We are fixing dsn for mysql server inside of db_MySQL package.
  #TODO: we should remove hack below and do the same things for other servers
  fix_dsn($server) unless ($name =~ /mysql/i);
...
sub all_servers
{
  return ["Access", "Adabas", "DB2", "Empress", "FrontBase", "Oracle",
	  ...
	  "SQLite","Solid", "Sybase-ASA","Sybase-ASE","Firebird"];
}
...
sub fix_dsn
{
  ...
  if (defined($main::opt_odbc) && $main::opt_odbc)
  {
...
sub new
{
  my ($type,$host,$database,$machine,$socket,$connect_options,$in_cmp, $driver)= @_;
  my $self= {};
  my %limits;
  bless $self;
  $host='localhost' if (!defined($host) or ($host eq ''));

  $self->{'cmp_name'}		= "$driver";
  $self->{'data_source'}	= "DBI:$driver:database=$database;host=$host";
  ...
  $limits{'fk'}                 = 0; # Doesn't support foreign keys.
  $limits{'views'}              = 0; # Doesn't support views      

  $limits{'union'}              = 0; # supports union      
  $limits{'union_all'}          = 0; # supports union all      
  $limits{'subqueries'}		= 0; # Doesn't support sub-queries.
  $limits{'multitable_delete'}  = 0; # supports multitable delete.
  
  #Fix DSN in case of using of ODBC driver
  main::fix_dsn($self);

  # Some fixes that depends on the environment
  # determine type of tables.
  if ($in_cmp || $main::opt_old_headers) 
  {
    if ( $main::opt_assume_mysql_tabletype =~ /^(myisam|falcon|innodb|gemini|isam|heap|bdb|pbxt)/i)
    { 
      $self->{'tabletype'} = $1;
    }
    else
    {
      $self->{'tabletype'} = 'myisam';
    }
    if ($main::opt_assume_mysql_version =~ /^(\d+)\.(\d+)\.(\d+)/)
    {
      $self->{'version'} = $3+$2*100+$1*10000;
    } 
    else
    {
      $self->{'version'} = 400114;
  ...
  {
    ( $self->{'tabletype'},
      $self->{'version'} ) = get_mysql_info($self->{'data_source'},
                                 $main::opt_user,$main::opt_password);
  }				     
  # Some fixes that depends on the environment
  if ($self->{'tabletype'} =~ /heap/i)
  {
    $limits{'working_blobs'}  = 0; # HEAP tables can't handle BLOB's
  }
  if ($self->{'tabletype'} =~ /InnoDB/i)
  {
    $self->{'transactions'}   = 1;    # Transactions enabled
    $limits{'fk'}             = 1;    # Supports fk
  }
  if ($self->{'tabletype'} =~ /Falcon/i)
  {
    $self->{'transactions'}   = 1;    # Transactions enabled
  }
  if ($self->{'tabletype'} =~ /PBXT/i)
  {
    $self->{'transactions'}   = 1;    # Transactions enabled
  }
  if ($self->{'tabletype'} =~ /ndb/i)
  {
    $self->{'transactions'}   = 1;    # Transactions enabled
    $limits{'max_columns'}    = 127;  # NDB only goes up to 128 columns
    $limits{'max_tables'}     = 500;  # should be safe
  }
  if ($self->{'tabletype'}  =~ /bdb/i)
  {
    $self->{'transactions'}   = 1;    # Transactions enabled
  }
  if ($self->{'tabletype'} =~ /gemini/i)
  {
    $limits{'working_blobs'}  = 0; # Blobs not implemented yet
    $limits{'max_tables'}     = 500;
    $self->{'transactions'}   = 1;    # Transactions enabled
  }
  if ($self->{'tabletype'} =~ /isam/i) #isam or myisam
  {
    # if the database must be big enough, we shall add max_rows to
    # the create table command
    if ($main::max_table_size > 500000) 
    {
      my $shortest_recordlen = 4; # in the test-insert shortest record is  bench2
      my $maxrows = int ($main::max_table_size / $shortest_recordlen);
      $main::opt_create_options .= " MAX_ROWS=$maxrows" unless ($in_cmp) ;
    } else {
 #       print "Too small tablesize =$main::max_table_size\n";
    };
  ...
  {
    $limits{'union'}              = 1; # supports union      
    $limits{'union_all'}          = 1; # supports union all          
  }

  if ($self->{'version'} >= 40002)
  {
    $limits{'multitable_delete'}  = 1; # supports multitable delete.
  }

  if ($self->{'version'} > 40100)
  {
    $limits{'subqueries'}              = 1; # support subselects
    $limits{'subqueries_select'}=1;   # subqueries in select list
    $limits{'subqueries_alias'}=1;    # aliases in subqueries 
    $limits{'subqueries_where'}=1;    # subqueries in 'where' part
    $limits{'subqueries_groupby'}=1;  # subqueries in group by
    $limits{'subqueries_having' }=1;  # subqueries in 'having' part
    $limits{'subqueries_orderby'}=1;  # subqueries in 'order by'
    $limits{'subqueries_from'}=1;     # subqueries in 'from' part
    $limits{'subqueries_all'}=1;      # ALL subqueries 
    $limits{'subqueries_some'}=1;     # SOME subqueries in select list
    $limits{'subqueries_any'}=1;      # ANY subqueries 
    $limits{'subqueries_correlated'}=1;      # correlated subqueries 

  }
  if ($self->{'version'} > 50001)
  {
    $limits{'views'}=1; 
  }
  if ($self->{'version'} > 60003)
  {
    $limits{'subqueries_in_correlated'} =
                               { old => 'set @@optimizer_switch=\'no_semijoin\'',
                                 sj  => 'set @@optimizer_switch=\'\''
                               };
    $limits{'subqueries_in_uncorrelated'} =
                               { mat =>'set @@optimizer_switch=\'no_semijoin\'',
                                 sj  =>'set @@optimizer_switch=\'no_materialization\'',
                                 old =>'set @@optimizer_switch=\'no_materialization,no_semijoin\''
                               };
  }
  else
  {
    $limits{'subqueries_in_correlated'} = { default =>'' };
    $limits{'subqueries_in_uncorrelated'} = { default =>'' };
  }
...
sub version
{
  ...
  $dbh=$self->connect();
  $sth = $dbh->prepare("select VERSION(),\@\@version_compile_machine") or die $DBI::errstr;
  $version="MySQL 3.20.?";
  if ($sth->execute && (@row = $sth->fetchrow_array))
  {
    $row[0] =~ s/-/ /g;			# To get better tables with long names
    $row[1] =~ s/\///g;
    $version="MySQL $row[0]/$row[1]";
  }
  $sth->finish;
...
sub get_mysql_info
{
   my ($data_src,$user,$passwd) = @_;
   my ($dbh,$sth,@row,$tabletype, $version);

   $dbh=DBI->connect($data_src, $user,$passwd,{ PrintError => 0}) ||
                     die "Got error: '$DBI::errstr' when connecting to " . $data_src.
		     "with user: '$user' password: '$passwrd'\n";

   $dbh->do("drop table if exists benchtabletest") or die $DBI::errstr;;
   $dbh->do("create table benchtabletest (s1 int) ".$main::opt_create_options) or die $DBI::errstr;;
   $sth = $dbh->prepare("show table status like 'benchtabletest'") or die $DBI::errstr;
   $tabletype="myisam";
   if ($sth->execute && (@row = $sth->fetchrow_array))
   {
     $tabletype=$row[1];
   ...
   $version =~ /^(\d+)\.(\d+)\.(\d+)/;
   $version = $3+$2*100+$1*10000;
   $dbh->do("drop table benchtabletest") or die $DBI::errstr;;
   $dbh->disconnect;
   return ($tabletype, $version);
}
...
sub create
{
  my($self,$table_name,$fields,$index,$options,$constraints) = @_;
  my($query,@queries);
  ...
  foreach $index (@$index)
  {
    $index =~ s/^(index|key|primary\s+key|unique)\s*([\w]+|)\s*\(/\1 \2 $main::opt_create_index_options (/i;
    $query.= $index . ',';
  }
  if ($constraints)
  {
...
sub vacuum
{
  ...
  if ($#tables >= 0)
  {
    ...
    print "Time for book-keeping (1): " .
      My::Timer::timestr(Benchmark::timediff($end_time, $loop_time),"all") . "\n\n";
  }
}
...
sub new
{
  ...
  $limits{'multi_distinct'}     = 1; # allows select count(distinct a),count(distinct b).. 
  $limits{'fk'}                 = 0; # Doesn't support foreign keys.
  $limits{'multitable_delete'}  = 0; # multitable delete.
  $limits{'views'}              = 0; # Doesn't support views      
  $limits{'union'}              = 0; # supports union      
  $limits{'union_all'}          = 0; # supports union all      
  $limits{'subqueries_select'}  = 0; # subqueries in select list
  $limits{'subqueries_alias'}   = 0; # aliases in subqueries 
  $limits{'subqueries_where'}   = 0; # subqueries in 'where' part
  $limits{'subqueries_groupby'} = 0; # subqueries in group by
  $limits{'subqueries_having' } = 0; # subqueries in 'having' part
  $limits{'subqueries_orderby'} = 0; # subqueries in 'order by'
  $limits{'subqueries_from'}    = 0; # subqueries in 'from' part
  $limits{'subqueries_all'}     = 0; # ALL subqueries 
  $limits{'subqueries_some'}    = 0; # SOME subqueries in select list
  $limits{'subqueries_any'}     = 0; # ANY subqueries 
  $limits{'subqueries_correlated'}=0;      # correlated subqueries 
...
sub new
{
  ...
  $self->{'data_source'} .= ";host=$host" if $host;
  $self->{'limits'}		= \%limits;
  $self->{'blob'}		= "text";
  $self->{'text'}		= "text";
  ...
  $limits{'working_blobs'}	= 1; # If big varchar/blobs works
  $limits{'multi_distinct'}     = 1; # allows select count(distinct a),count(distinct b).. 
  $limits{'fk'}                 = 1; # Supports foreign keys.
  $limits{'multitable_delete'}  = 0; # multitable delete.
  $limits{'views'}              = 1; # Supports views      
  $limits{'union'}              = 1; # supports union      
  $limits{'union_all'}          = 1; # supports union all      
  $limits{'subqueries_select'}  = 1; # subqueries in select list
  $limits{'subqueries_alias'}   = 0; # aliases in subqueries 
...
{
  my ($self)=@_;
  my ($dbh,$sth,$version,@row, $dir);

  #Trying to detect version of server
  ...
  return $version if ($version ne "PostgreSQL version ");
...
#    $field =~ s/ date/ int/i;
  ...
  foreach $index (@$index)
  {
    ...
    push(@queries,"create $in ${table_name}_$table on $table_name $main::opt_create_index_options $indfield");
  }
  $queries[0]=$query;
  return @queries;
}
...
sub new
{
  ...
  $limits{'multi_distinct'}     = 1; # allows select count(distinct a),count(distinct b).. 
  $limits{'fk'}                 = 1; # Supports foreign keys.
  $limits{'multitable_delete'}  = 1; # multitable delete.
  $limits{'views'}              = 1; # Doesn't support views      
  $limits{'union'}              = 1; # supports union      
  $limits{'union_all'}          = 1; # supports union all      
  $limits{'subqueries_select'}  = 1; # subqueries in select list
  $limits{'subqueries_alias'}   = 1; # aliases in subqueries 
...
#############################################################################
#	     Configuration for Sybase ASE
#############################################################################
package db_sybase_ase;
...
#############################################################################
#	     Configuration for Sybase ASA
#############################################################################
package db_sybase_asa;
...
sub new
{
  ...
  $self->{'transactions'}	= 1; # Transactions enabled
  $self->{"vacuum"}		= 1;
  $self->{'vendor'}             = "Sybase_ASA";

  $limits{'max_conditions'}	= 1030; # We get 'Query is too complex'
  $limits{'max_columns'}	= 8162;	# Max number of columns in table
  $limits{'max_tables'}		= 65000;	# Should be big enough
  $limits{'max_text_size'}	= 262144;  # Max size with default buffers.
  $limits{'query_size'}		= 16777216; # Max size with default buffers.
  $limits{'max_index'}		= 64; # Max number of keys
  $limits{'max_index_parts'}	= 64; # Max segments/key
...
{
  my($self,$table_name,$fields,$index,$options,$constraints) = @_;
  my($query,@queries,$ind,@keys, @indx);
...
sub new
{
  ...
  $self->{'cmp_name'}		= "firebird";
  $self->{'data_source'}	= "DBI:InterBase:$database";
  $self->{'limits'}		= \%limits;
  $self->{'blob'}		= "blob";
  $self->{'text'}		= "";
  $self->{'double_quotes'}	= 1; # Can handle:  'Walker''s'
  ...
  $self->{'char_null'}          = "";
  $self->{'numeric_null'}       = "";
  $self->{'vendor'}             = "Firebird";

  $limits{'max_conditions'}	= 2944; # (Actually not a limit)
  $limits{'max_columns'}	= 252;	# Max number of columns in table
  $limits{'max_tables'}		= 65000;	# Should be big enough
  $limits{'max_text_size'}	= 15000; # Max size with default buffers.
  $limits{'query_size'}		= 1000000; # Max size with default buffers.
  $limits{'max_index'}		= 65000; # Max number of keys
  $limits{'max_index_parts'}	= 8; # Max segments/key
  $limits{'max_column_name'}	= 31; # max table and column name
...
sub insert_file {
  my($self,$dbname, $file) = @_;
  print "insert of an ascii file isn't supported by Firebird\n";
  return 0;
}
...
sub new
{
  my ($type,$host,$database, $socket, $connect_options, $name)= @_;
  my $self= {};
  my %limits;
  bless $self;
  $host='localhost' if (!defined($host) or ($host eq ''));
  ...
  if ($driver =~ /maxdb$/i || $driver =~ /sap_db/i)
  {
    $self->{'data_source'}	= "DBI:$driver:$database";

    # Do not use 'localhost' as hostname because it switches to tcp loopback on MaxDB
    if ($host ne "localhost")
    {
      $self->{'data_source'}	.= ":$host";
    }
  }
  elsif ($driver =~ /maxdbproxy/i)
  {
  ...
  $limits{'max_conditions'}	= 508; # (Actually not a limit) *
  $limits{'max_columns'}	= 1023;	# Max number of columns in table *
  $limits{'max_tables'}		= 65000;	# Should be big enough * unlimited actually
  $limits{'max_text_size'}	= 8024; # Max size with default buffers. 
  $limits{'query_size'}		= 32552; # Max size with default buffers. 
...
# Get the version number of the database
#
# TODO: add version detection for maxdbproxy  
...
sub vacuum
{
  ...
  if ($#tables >= 0)
  {
    ...
    foreach $table (@tables)
    {
        $dbh->do("update stat $table estimate sample 10 percent") or die "Got error: $DBI::errstr when executing '$command'\n";
    }
...
sub new
{
  ...
  $limits{'max_conditions'}	= 9999; # (Actually not a limit) 
  $limits{'max_columns'}	= 8192;	# Max number of columns in table 
  $limits{'max_tables'}		= 2**32;	# 2^32 theoretically.
  $limits{'max_text_size'}	= 1048569; # Max size with default buffers. 
  $limits{'query_size'}		= 16777216; # Max size with default buffers. *64 kb by default. May be set by system variable 
  $limits{'max_index'}		= 2**32; # Max number of keys 2^32 theoretically.
  $limits{'max_index_parts'}	= 64; # Max segments/key 
  $limits{'max_column_name'}	= 512; # max table and column name  
  ...
  $limits{'group_func_sql_min_str'} = 1; # Can execute MIN() and MAX() on strings 
  $limits{'group_distinct_functions'}= 0; # Have count(distinct)
  $limits{'select_without_from'}= 1; # Cannot do 'select 1';
  $limits{'multi_drop'}		= 0; # Drop table cannot take many tables 
  $limits{'subqueries'}		= 1; # Supports sub-queries. 
  $limits{'left_outer_join'}	= 1; # Supports left outer joins 
  $limits{'table_wildcard'}	= 1; # Has SELECT table_name.
  ...
  $limits{'like_with_column'}	= 1; # Can use column1 LIKE column2 
  $limits{'order_by_position'}  = 1; # Can use 'ORDER BY 1' 
  $limits{'group_by_position'}  = 1; # Cannot use 'GROUP BY 1' 
  $limits{'alter_table'}	= 0; # Have ALTER TABLE 
  $limits{'alter_add_multi_col'}= 0; # Have ALTER TABLE t add a int,add b int; 
  $limits{'alter_table_dropcol'}= 0; # Have ALTER TABLE DROP column  
  $limits{'insert_multi_value'} = 0; # INSERT ... values (1,2),(3,4) 
  ...
  $limits{'func_odbc_mod'}	= 0; # Have function mod. *
  $limits{'func_extra_%'}	= 1; # Does not have % as alias for mod() *
  $limits{'func_odbc_floor'}	= 0; # Has func_odbc_floor function *
  $limits{'func_extra_if'}	= 0; # Does not have function if. *
  $limits{'column_alias'}	= 1; # Alias for fields in select statement. *
  $limits{'NEG'}		= 1; # Supports -id *
  $limits{'func_extra_in_num'}	= 1; # Has function in *
  $limits{'limit'}		= 1; # Does not support the limit attribute *
  $limits{'working_blobs'}	= 1; # If big varchar/blobs works *
  $limits{'order_by_unused'}	= 1; # 
  $limits{'working_all_fields'} = 1; #
  $limits{'multi_distinct'}     = 0; # allows select count(distinct a),count(distinct b).. 
  $limits{'fk'}                 = 0; # Supports foreign keys.
  $limits{'multitable_delete'}  = 0; # multitable delete.
  $limits{'views'}              = 1; # Supports views      
  $limits{'union'}              = 1; # supports union      
  $limits{'union_all'}          = 1; # supports union all      
...
sub connect
{
  my ($self)=@_;
  my ($dbh);
  $dbh=DBI->connect($self->{'data_source'},"","",{ PrintError => 0, AutoCommit => 1}) ||
		      die "Got error: '$DBI::errstr' when connecting to " . $self->{'data_source'} ." with user: '$main::opt_user' password: '$main::opt_password'\n";
...
sub insert_file {
  my($self,$dbname, $file) = @_;
  print "insert of an ascii file isn't supported by SQLITE\n";
  return 0;
}
