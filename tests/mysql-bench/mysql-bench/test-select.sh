...
if ($opt_lock_tables)
{
  do_query($dbh,"LOCK TABLES bench1 WRITE, bench2 WRITE");
}
...
print "Inserting ",$opt_loop_count*2," rows into 2 tables\n";
...
{
  $region=chr(65+($region_id++));
  do_query($dbh,"insert into bench2 values('$region',$idn)");
  if ($region_id >= $opt_regions) {
    $idn++;
    $region_id=0;
  }
}
...
print "Time to insert (",$opt_loop_count*2,"): " .
...
if ($limits->{'group_functions'})
{
  ...
  $loop_time=My::Timer::get_timer();
  for ($tests=0 ; $tests < $opt_medium_loop_count ; $tests++)
  {
    fetch_all_rows($dbh,"select sum(idn+100),sum(rev_idn-100) from bench1");
  }
  $end_time=My::Timer::get_timer();
  print "Time for select_cache ($opt_medium_loop_count): " .
     My::Timer::timestr(timediff($end_time, $loop_time),"all") . "\n\n";
  ...
  $end_time=My::Timer::get_timer();
  print "Time for select_cache2 ($opt_medium_loop_count): " .
     My::Timer::timestr(timediff($end_time, $loop_time),"all") . "\n\n";
}
...
$rows=0;
$count=0;
for ($i=0 ; $i < $opt_small_loop_count*10 ; $i++)
{
  $grp=$i*11 % $opt_groups;
...
if ($limits->{'group_functions'} && $limits->{'order_by_unused'})
{
  ...
  {
    $region=chr(65+$tests % $opt_regions);
    $idn = $max_idn_bench2 / 2;
    fetch_all_rows($dbh,"select max(idn) from bench2 where region='$region'".
                     " and idn between 1 and $idn");
    fetch_all_rows($dbh,"select min(idn) from bench2 where region='$region'".
                     " and idn > $idn");                   
    $end_time=My::Timer::get_timer();;
    $count += 2;
    last if ($estimated=predict_query_time($loop_time,$end_time,\$count,
  ...
  print_time($estimated);
  print " for min_max_on_key2 ($count): " .
     My::Timer::timestr(timediff($end_time, $loop_time),"all") . "\n";

  $loop_time=My::Timer::get_timer();;
  $count=0;
  $estimated=0;
  for ($tests=0 ; $tests < $opt_loop_count /3 ; $tests++)
  {
    $count+=5;
    $grp=$tests*3 % $opt_groups;
    $region=chr(65+$tests % $opt_regions);
...
if (!$opt_skip_delete)
{
  ...
  do_query($dbh,"drop table bench2" . $server->{'drop_attr'});
}
