...
if (exists($server->{tabletype}))
{
  $opt_comments.= "Table type: $server->{tabletype}";
}
...
print "Filesystem:          $opt_fs\n";
print "Vendor:              $server_vendor\n\n";
...
while (<test-*>)
{
  ...
  if ($last_line =~ /Total time:/i)
  {
    ...
    print " Done: $prog_sum[0] tests ". lcfirst($last_line) ."\n";
  }
...
sub read_headers
{
  ...
  $benchmark_version=$date=$machine=$server_version=$server_vendor="";
