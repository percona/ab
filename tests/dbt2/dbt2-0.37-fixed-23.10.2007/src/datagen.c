...
void gen_stock()
{
	...
	printf("Finished stock table data... before close\n");
	fclose(output);
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "c:d:i:n:o:w:T:",
...
//        return 0;
	...
	gen_customers();
	} else if (strcmp(dt, "h") == 0) {
	gen_history();
	} else if (strcmp(dt, "o") == 0) {
	gen_orders();
	} else if (strcmp(dt, "n") == 0) {
	gen_new_orders();
	} 
	return 0;
}
